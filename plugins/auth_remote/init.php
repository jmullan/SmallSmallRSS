...
class Auth_Remote extends \SmallSmallRSS\Plugin implements \SmallSmallRSS\Auth_Interface {
    ...
    function get_login_by_ssl_certificate() {
        ...
        if ($cert_serial) {
            $result = \SmallSmallRSS\Database::query("SELECT login FROM ttrss_user_prefs, ttrss_users
				WHERE pref_name = 'SSL_CERT_SERIAL' AND value = '$cert_serial' AND
				owner_uid = ttrss_users.id");

            if (\SmallSmallRSS\Database::num_rows($result) != 0) {
                return \SmallSmallRSS\Database::escape_string(\SmallSmallRSS\Database::fetch_result($result, 0, "login"));
            }
        }
    ...
    function authenticate($login, $password) {
        ...
        // php-cgi
        if (!$try_login) $try_login = \SmallSmallRSS\Database::escape_string($_SERVER["REDIRECT_REMOTE_USER"]);
            ...
            if ($user_id) {
                ...
                if (\SmallSmallRSS\Config::get('AUTH_AUTO_CREATE')) {
                    ...
                    if ($fullname) {
                        ...
                        \SmallSmallRSS\Database::query("UPDATE ttrss_users SET full_name = '$fullname' WHERE id = " .
                                 $user_id);
                    }
                    // update user mail
                    ...
                    if ($email) {
                        ...
                        \SmallSmallRSS\Database::query("UPDATE ttrss_users SET email = '$email' WHERE id = " .
                                 $user_id);
                    }
                }
