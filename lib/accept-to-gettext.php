...
function al2gt($gettextlangs, $mime) {
    ...
    foreach ($gettextlangs as $gtlang) {
        ...
        $gtcs = isset($tmp2[1]) ? strtoupper($tmp2[1]) : '';
        ...
        if (isset($alscores[$allang])) {
            if (isset($acscores[$gtcs])) {
                $testvals[] = array($alscores[$allang], $acscores[$gtcs]);
            }
            $testvals[] = array($alscores[$allang], $acscores["*"]);
        }
        if (isset($alscores[$noct[0]])) {
            if (isset($acscores[$gtcs])) {
                $testvals[] = array($alscores[$noct[0]], $acscores[$gtcs]);
            }
            $testvals[] = array($alscores[$noct[0]], $acscores["*"]);
        }
        if (isset($alscores["*"])) {
            if (isset($acscores[$gtcs])) {
    ...
    $gtparts = explode(".", $curgtlang);
    $tmp = strtolower($gtparts[0]);
    $lang = preg_replace("/\_/", "-", $tmp);
    $charset = isset($gtparts[1]) ? $gtparts[1] : '';
