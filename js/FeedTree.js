...
dojo.declare("fox.FeedTree", dijit.Tree, {
    ...
    getRowClass: function (item, opened) {
        var rc = (!item.error || item.error == '') ? "dijitTreeRow" : "dijitTreeRow Error";
                    ...
                    if (hide && cat_unread == 0 && check_unread == 0) {
                        Effect.Fade(
                            node[0].rowNode, {
                                duration : 0.3,
                                queue: {
                                    position: 'end', scope: 'FFADE-' + id, limit: 1 }});
                    } else {
    ...
    hideRead: function (hide, show_special) {
        if (this.hasCats()) {
            ...
            this.hideReadFeeds(this.model.store._arrayOfTopLevelItems, hide, show_special);
        }
    },
    ...
    getNextFeed: function (feed, is_cat) {
        ...
        if (item) {
            return [
                this.model.store.getValue(item, 'bare_id'),
    ...
    getFeedCategory: function(feed) {
        try {
            return this.getNodesByItem(this.model.store._itemsByIdentity["FEED:" + feed])[0].getParent().item.bare_id[0];
        } catch (e) {
            return false;
        }
    },
